    ...
    <div className="w-full h-[480px] rounded-2xl overflow-hidden border border-white/10 bg-black/50 backdrop-blur">
      <div className="w-full h-full">
        <div key={ids[1]} className="w-full h-full">
          <DailyVideo automirror={false} sessionId={ids[1]} className="w-full h-full object-cover" />
...
function detectType(text) {
  ...
  if (/(shelter|housing|bed|navigation|transitional|family shelter)/i.test(t)) return "Housing/Shelter";
  if (/(clinic|health|medical|care|vaccin|primary|wellness|dental)/i.test(t)) return "Clinic";
  return null;
}
...
export default function Console() {
  ...
  const filteredBase = useMemo(() => {
    ...
    let base = all.filter((r) => {
      ...
      if (!kw) return true;
      const blob = `${r.name} ${r.description} ${r.category} ${r.services_offered} ${r.address1} ${r.city} ${r.zip}`.toLowerCase();
  ...
  function matchResourcesByPhrase(phrase, pool) {
    ...
    const scored = pool.map((r) => {
      const blob = `${r.name} ${r.description} ${r.category} ${r.services_offered}`.toLowerCase();
      let score = 0;
      ...
      if (/food|grocer|pantry|bank/.test(p) && /food/i.test(r.category + " " + r.services_offered)) score += 2;
      if (/shelter|housing|bed/.test(p) && /housing|shelter/i.test(r.category)) score += 2;
      if (/clinic|health|medical|care/.test(p) && /clinic|health/i.test(r.category)) score += 2;
  ...
  function setHighlightsByName(name) {
    ...
    const ids = filteredBase.filter((r) => r.name.toLowerCase().includes(n)).map((r) => r.id).slice(0, 5);
    setHighlightedIds(new Set(ids));
    if (ids.length) focusHighlighted(ids[0]);
  }
  ...
  function wireDailyEvents(co) {
    co.on("app-message", (ev) => {
      ...
      if (d.event_type === "conversation.utterance" && d.properties?.speech) phrases = [d.properties.speech];
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="pt-6 pb-4">
          ...
          <div className="mt-3 flex items-center gap-2 text-xs">
            ...
            <button
              className={"px-2 py-1 rounded-lg " + (!activeType ? "bg-white/20" : "bg-white/10")}
              onClick={() => {
                setActiveType(null);
                setPage(1);
              }}
            ...
            <button
              className={"px-2 py-1 rounded-lg " + (activeType === "Food Bank" ? "bg-white/20" : "bg-white/10")}
              onClick={() => {
            ...
            <button
              className={"px-2 py-1 rounded-lg " + (activeType === "Housing/Shelter" ? "bg-white/20" : "bg-white/10")}
              onClick={() => {
            ...
            <button
              className={"px-2 py-1 rounded-lg " + (activeType === "Clinic" ? "bg-white/20" : "bg-white/10")}
              onClick={() => {
          ...
          <div>
                ...
                return (
                  <div
                    ...
                    className={
                      ...
                      (isHighlighted ? "border-[#2ED2C9] ring-2 ring-[#2ED2C9]/50 scale-[1.01]" : "border-white/10")
                    }
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <a href={r.website || "#"} target="_blank" rel="noreferrer" className="text-xs px-3 py-1 rounded-lg bg-white/10 hover:bg-white/20">
                        Website
                      </a>
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-[10px]">
                      {r.cost ? <span className="px-2 py-1 rounded bg-white/10">{r.cost}</span> : null}
                      {r.languages ? <span className="px-2 py-1 rounded bg-white/10">{r.languages}</span> : null}
                      {r.appointment_required ? <span className="px-2 py-1 rounded bg-white/10">Appt: {r.appointment_required}</span> : null}
                      {r.walk_in_ok ? <span className="px-2 py-1 rounded bg-white/10">Walk-in: {r.walk_in_ok}</span> : null}
                      {r.capacity ? <span className="px-2 py-1 rounded bg-white/10">{r.capacity}</span> : null}
                      {r.transit ? <span className="px-2 py-1 rounded bg-white/10">{r.transit}</span> : null}
                    </div>
                  </div>
                );
              })}
            ...
            <div className="mt-6 flex items-center justify-between">
              <div className="text-sm text-white/60">
                Showing {(page - 1) * pageSize + 1}-{Math.min(page * pageSize, filteredBase.length)} of {filteredBase.length}
